...
main(int argc, char **argv)
{
	...
	keyloc = (char**)malloc(sizeof(keyloc));
	keyloc[0] = (char*)malloc(7);
	sprint(keyloc[0], "rsakey");
	if(argc == 1)
		rsa = getkey(1, keyloc, 1, 0);
	else
		rsa = getkey(1, &argv[1], 1, 0);
	if(rsa == nil)
		sysfatal("priv read fail");
	Binit(&b, 0, OREAD);
	clr = mpnew(0);
	enc = mpnew(0);
